...
-- Travel Options
local function addInstantTravel(keywordHandler, destination, cost, position, condition)
    ...
    keywordHandler:addKeyword(keywords, function(npc, player, message, keywords, parameters, node)
        if player:isPlayer() then
            if condition and not condition(player) then
                npcHandler:say("I'm sorry, but I can't take you to " .. destination .. " at the moment.", npc, player)
                return true
            end
...
local function addTravelKeyword(keyword, cost, destination, action, condition)
    local keywords = {"bring me to " .. keyword:lower()}
    keywordHandler:addKeyword(keywords, StdModule.say, { npcHandler = npcHandler, text = "I'm sorry but I don't sail there." }, condition)
    local travelKeyword = keywordHandler:addKeyword(keywords, StdModule.say, { npcHandler = npcHandler, text = "Do you seek a passage to " .. keyword:titleCase() .. " for |TRAVELCOST|?", cost = cost, discount = "postman" })
...
local function canTravelToYalahar(player)
...
addInstantTravel(keywordHandler, "Yalahar", 200, Position(32816, 31272, 6), canTravelToYalahar)
...
addTravelKeyword("yalahar", 200, Position(32816, 31272, 6), nil, canTravelToYalahar)
